...
class SQSClientExtended(object):
    ...
    def receive_message(self,
        ...
        for message in opt_messages:
            large_pay_load_attribute_value = message.get(
    ...
    def __delete_message_payload_from_s3(self, receipt_handle):
        try:
            ...
            s3_object.delete()
            print('Deleted s3 object https://s3.amazonaws.com/{}/{}'.format(
                s3_msg_bucket_name, s3_msg_key))
        except Exception as e:
            print(
                "Failed to delete the message content in S3 object. {}, type:{}"
                .format(str(e),
                        type(e).__name__))
            raise e
    ...
    def __get_orig_receipt_handle(self, receipt_handle):
        return receipt_handle[receipt_handle.rindex(SQSExtendedClientConstants.
                                                    S3_KEY_MARKER.value) +
                              len(SQSExtendedClientConstants.S3_KEY_MARKER.
                                  value):]
    ...
    def _send_queue_message(self, queue_url, message_body, message_group_id,
    ...
    def send_message(self,
        ...
        if msg_attributes_size > self.message_size_threshold:
            raise ValueError(
                "Total size of Message attributes is {} bytes which is larger than the threshold of {} Bytes. Consider including the payload in the message body instead of message attributes."
        ...
        if large_payload_attribute_value:
            raise ValueError(
                "Message attribute name {} is reserved for use by SQS extended client."
                .format(
                    SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value))

        if self.always_through_s3 or self.__is_large(str(message),
                                                     message_attributes):
            ...
            message_attributes[
                SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value] = {
                    'StringValue':
                    str(self.__get_string_size_in_bytes(message)),
                    'DataType': 'Number'
                }
            return self._send_queue_message(queue_url, s3_key_message,
                                            message_group_id,
    ...
    def __store_message_in_s3(self, message_body):
        ...
        try:
            ...
            session = Session(aws_access_key_id=self.aws_access_key_id,
                              ...
                              region_name=self.aws_region_name)
            s3 = session.resource('s3')
            opt_file = tempfile.NamedTemporaryFile(mode='w+',
                                                   encoding='utf-8',
                                                   delete=False)
            opt_file.write(str(message_body))
            opt_file.flush()
            reader = open(opt_file.name, mode='r', encoding='utf-8')
            s3.Bucket(self.s3_bucket_name).put_object(Key=s3_key,
                                                      Body=reader.read())
            reader.close()
            opt_file.close()
            ...
            print(
                "Failed to store the message content in an S3 object. SQS message was not sent. {}, type:{}"
                .format(str(e),
                        type(e).__name__))
            raise e
